...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Backfill 1-minute bars for a list of symbols from Alpaca API and store as partitioned Parquet files."
    )
    ...
    ap.add_argument(
        "--feed", default=None, help="Override feed (default uses env ALPACA_FEED or 'iex')"
    )
    ap.add_argument(
        "--max-symbols",
        type=int,
    ...
    ap.add_argument(
        ...
        type=int,
        default=0,
        help="Sleep time in milliseconds between API calls (default: 0ms)",
    )
    ...
    failures_path = (
        report_dir
        / f"backfill_1m_failures_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}.csv"
    )
    ...
    for day_start in day_iter:
        ...
        sym_iter = (
            symbols
            if args.no_progress
            else tqdm(symbols, desc=f"Symbols ({day_str})", leave=False, unit="sym")
        )
        ...
        for sym in sym_iter:
            try:
                ...
                if not norm.empty:
                    write_partitioned_parquet(norm, writer_cfg, partition_cols=["symbol", "date"])
    ...
    log.info(
        f"Done. Days processed: {len(days)} | Symbols: {len(symbols)} | Elapsed: {elapsed_all}"
    )
