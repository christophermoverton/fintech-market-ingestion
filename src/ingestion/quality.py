...
def compute_duplicates(
    con: duckdb.DuckDBPyConnection, cfg: QAConfig
    ...
    by_symbol = (
        ...
        .groupby("symbol")
        .agg(duplicate_keys=("key_count", "size"), duplicate_rows=("extra_rows", "sum"))
...
def compute_ohlc_violations(
    ...
    violations_samples = (
        pd.concat(samples_frames, ignore_index=True) if samples_frames else pd.DataFrame()
    )
...
def compute_gaps_1m(con: duckdb.DuckDBPyConnection, cfg: QAConfig) -> pd.DataFrame:
    ...
    if df.empty:
        return pd.DataFrame(
            columns=["symbol", "date", "rows", "expected_minutes", "missing_minutes"]
        )
...
def run_qa(cfg: QAConfig) -> int:
    ...
    cols = (
        con.execute(f"SELECT * FROM read_parquet('{cfg.parquet_glob}') LIMIT 0")
        .df()
        .columns.tolist()
    )
    ...
    if not dup_by_symbol.empty:
        summary = summary.merge(
            dup_by_symbol[["symbol", "duplicate_keys", "duplicate_rows"]], on="symbol", how="left"
        )
    ...
    if not ohlc_summary.empty:
        _write_csv(
            ohlc_summary.sort_values(["violation_count"], ascending=False),
    ...
    if cfg.strict:
        ...
        if total_ohlc_violations > cfg.max_ohlc_violations:
            fail_reasons.append(
                f"ohlc_violations={total_ohlc_violations} > {cfg.max_ohlc_violations}"
            )
        if missing_cols:
        ...
        if fail_reasons:
            ...
            print("See reports/qa_summary.csv and conditional reports for details.")
            return 1
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Run QA checks for curated market data (daily + 1-minute bars)."
    )
    ap.add_argument(
        "--timeframe",
        choices=["1D", "1Min", "all"],
