...
def main():
    ap = argparse.ArgumentParser(
        description="Backfill daily bars for a list of symbols from Alpaca API and store as partitioned Parquet files."
    )
    ...
    ap.add_argument(
        ...
        choices=["month", "year"],
        default="month",
        help="Window size for API requests (default: month)",
    )
    args = ap.parse_args()
    ...
    log.info(
        f"Symbols: {len(symbols)} | Range: {start_ts.date()} -> {end_ts.date()} | Windows: {len(windows)}"
    )
