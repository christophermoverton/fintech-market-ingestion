...
def _to_utc(ts: pd.Series) -> pd.Series:
    ...
    if out.isna().any():
        raise ValueError(f"Some timestamps could not be parsed. Check the input data: {out.isna().sum()} invalid timestamps.")
    return out
...
def normalize_bars(
    ...
    out = out.drop_duplicates(subset=["symbol", "ts_utc","timeframe"], keep="last").reset_index(drop=True)
